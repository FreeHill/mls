...
$qryy=mysql_fetch_array(mysql_query("select * from mlm_register where user_profileid='$_REQUEST[id]'"));

$mems=mysql_fetch_array(mysql_query("select * from mlm_membership where id='$qryy[user_membership]'"));
$user_sponserid=$qryy['user_sponserid'];
$user_profileid=$qryy['user_profileid'];

//echo $mems['act_amount']; exit;

$product_price=currency('USD','USD',$mems['act_amount']);

$item_name=$mems['membership_name'];

$trip_type="2";
$item_number=$qryy['user_membership'];

$return_url=$website_url."/success.php?id=$x_ordid&user_sponserid=$user_sponserid&user_profileid=$user_profileid";

$cancel=$website_url."/cancel.php?id=$x_ordid";
