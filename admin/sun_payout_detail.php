...
echo "<script>window.location='user.php?del';</script>";
 ...
 ?>
			...
			<div class="main-content">
				<div class="breadcrumbs" id="breadcrumbs">
					<ul class="breadcrumb">
						...
						<li>
							<a href="javascript:window.history.back()">Payout details</a>

							<span class="divider">
								<i class="icon-angle-right"></i>
							</span>
						</li>
						<li class="active"><?=$_REQUEST['detail']?></li>
					</ul><!--.breadcrumb-->
						  ...
						  ?> 
						   ...
						   ?>
							...
							<div class="row-fluid">
							 ...
							 Payout [<?php echo date("d-m-Y",strtotime($_REQUEST['detail'])); ?>]
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									<thead>
										...
										<th width="113"> Referrel Bonus</th>
	                                	 
										  <th width="113"> Indirect  Bonus</th>
										  
										  <th width="113"> Purchase Bouns </th> 
										  
	                                	 <th width="113"> Indirect Purchase Bonus </th>
	...
	$payy=mysql_query("select * from mlm_payoutcalc where pay_date='$_REQUEST[detail]' group by pay_user order by pay_id desc");
	$i=1;
	...
	{
	$refamt=$refamt+$drp['pay_amount'];
	}
	
	
	$inpuramt="";
	$inpurbonus=mysql_query("select * from mlm_payoutcalc where pay_date='$_REQUEST[detail]' and bonus_type='3' and pay_user='$reg[user_profileid]'" );
	
	while($inrp=mysql_fetch_array($inpurbonus))
	{
	$inpuramt=$inpuramt+$inrp['pay_amount'];
	}
	
	$inrefamt="";
	$inrefbonus=mysql_query("select * from mlm_payoutcalc where pay_date='$_REQUEST[detail]' and bonus_type='2' and pay_user='$reg[user_profileid]'" );
	
	while($indrp=mysql_fetch_array($inrefbonus))
	{
	$inrefamt=$inrefamt+$indrp['pay_amount'];
	}
?>
								...
								else
								{
									...
									$profileproof_image="images/nouser.png";
											...
											<td><?php echo $row_payy['pay_user']; ?></td>
											  ...
											  <td><?php if($inrefamt!=""){ echo $inrefamt; } else { echo "0"; }?></td>
											  
											  <td><?php if($puramt!=""){ echo $puramt; } else { echo "0"; } ?></td>
											  
											  <td><?php if($inpuramt!=""){ echo $inpuramt; } else { echo "0"; } ?></td>
											  
											 <td>
											  <a href="sun_details.php?piddd=<?php echo $row_payy['pay_user'];?>&date=<?php echo $_REQUEST['detail']; ?>" >
