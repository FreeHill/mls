...
if(isset($_REQUEST['submit']))
{
	...
	$docfile=mysql_real_escape_string($_FILES['docfile']['name']);
	...
	//$img_size = filesize($_FILES['docfile']['tmp_name']);
	//echo $img_size;exit;
 ...
 $insert = mysql_query("INSERT INTO mlm_document(doc_heading,doc_name,doc_date,doc_ip) VALUES ('$title','$cate_img_small',NOW(),'$ip')");
...
if(isset($_REQUEST['update']))
{
	...
	$doc_id = mysql_real_escape_string($_REQUEST['docid']);
	$title = mysql_real_escape_string($_REQUEST['title']);
	$docfile=mysql_real_escape_string($_FILES['docfile']['name']);
	$ip=$_SERVER['REMOTE_ADDR'];
	//echo $feature_image; //
...
if($title == "")
  ...
  // echo "UPDATE mlm_document SET doc_heading='$title' $docimg WHERE doc_id=$doc_id"; exit;
	
	$update = mysql_query("UPDATE mlm_document SET doc_heading='$title',doc_ip='$ip'$docimg WHERE doc_id=$doc_id");
	if($update) {
...
$act=mysql_query("update mlm_document set doc_status='1' where doc_id='$id'");
...
$act=mysql_query("update mlm_document set doc_status='0' where doc_id='$id'");
...
$det=mysql_query("delete from mlm_document where doc_id='$id'");
                           ...
                           <form action="" method="post">
							<div class="row-fluid">
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									...
									$docs=mysql_query("select * from mlm_document order by doc_id desc");
									$i=1;
									$num=mysql_num_rows(mysql_query("select * from mlm_document"));
									
									while($row_docs=mysql_fetch_array($docs))
									{
									?>
